...
END;
$$;
CREATE TRIGGER trg_recalculate_on_task_member_delete AFTER DELETE ON public.task_members FOR EACH ROW EXECUTE FUNCTION public.handle_member_removal_recalculate();
CREATE TRIGGER trg_recalculate_on_workspace_member_delete AFTER DELETE ON public.workspace_members FOR EACH ROW EXECUTE FUNCTION public.handle_member_removal_recalculate();
...
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Task not found.' USING ERRCODE = '22000'; END IF;
...
END;
$$;
CREATE TRIGGER trg_validate_task_member BEFORE INSERT OR UPDATE ON public.task_members FOR EACH ROW EXECUTE FUNCTION public.validate_task_member_insert();
...
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
DECLARE v_task_ws_id UUID;
BEGIN
  IF NEW.user_id IS NULL OR NEW.user_id <> auth.uid() THEN RAISE EXCEPTION 'Unauthorized: user_id must match authenticated user identity.' USING ERRCODE = '42501'; END IF;
  IF NEW.duration_seconds < 0 OR NEW.duration_seconds > 86400 THEN RAISE EXCEPTION 'Invalid duration_seconds.' USING ERRCODE = '22023'; END IF;
  IF NEW.task_id IS NOT NULL THEN
    IF NOT public.can_view_task(NEW.task_id, NEW.user_id) THEN RAISE EXCEPTION 'Unauthorized: User does not have access to specified task.' USING ERRCODE = '42501'; END IF;
    SELECT workspace_id INTO v_task_ws_id FROM public.tasks WHERE id = NEW.task_id;
    IF NEW.workspace_id IS NOT NULL AND v_task_ws_id IS NOT NULL AND NEW.workspace_id <> v_task_ws_id THEN RAISE EXCEPTION 'Mismatch: task_id does not belong to specified workspace_id.' USING ERRCODE = '22000'; END IF;
  END IF;
  IF NEW.workspace_id IS NOT NULL AND NOT public.is_workspace_member(NEW.workspace_id, NEW.user_id) THEN RAISE EXCEPTION 'Unauthorized: User is not a member of specified workspace.' USING ERRCODE = '42501'; END IF;
  RETURN NEW;
END;
$$;
CREATE TRIGGER trg_validate_focus_session BEFORE INSERT OR UPDATE ON public.focus_sessions FOR EACH ROW EXECUTE FUNCTION public.validate_focus_session();
...
CREATE POLICY "Members can view workspace" ON public.workspaces FOR SELECT TO authenticated USING (created_by = auth.uid() OR public.is_workspace_member(id, auth.uid()));
CREATE POLICY "Users can create workspace" ON public.workspaces FOR INSERT TO authenticated WITH CHECK (created_by = auth.uid());
CREATE POLICY "Owners can update workspace" ON public.workspaces FOR UPDATE TO authenticated USING (public.is_workspace_owner(id, auth.uid()));
CREATE POLICY "Owners can delete workspace" ON public.workspaces FOR DELETE TO authenticated USING (public.is_workspace_owner(id, auth.uid()));
...
CREATE POLICY "Members can view member list" ON public.workspace_members FOR SELECT TO authenticated USING (user_id = auth.uid() OR public.is_workspace_member(workspace_id, auth.uid()));
CREATE POLICY "Owners can add members" ON public.workspace_members FOR INSERT TO authenticated WITH CHECK (public.is_workspace_owner(workspace_id, auth.uid()));
CREATE POLICY "Owners or self can remove member" ON public.workspace_members FOR DELETE TO authenticated USING (user_id = auth.uid() OR public.is_workspace_owner(workspace_id, auth.uid()));
...
CREATE POLICY "Users can view permitted tasks" ON public.tasks FOR SELECT TO authenticated USING (public.can_view_task(id, auth.uid()));
CREATE POLICY "Users can create tasks" ON public.tasks FOR INSERT TO authenticated WITH CHECK (creator_id = auth.uid() AND (workspace_id IS NULL OR public.is_workspace_member(workspace_id, auth.uid())));
CREATE POLICY "Creators or workspace owners can update tasks" ON public.tasks FOR UPDATE TO authenticated USING (creator_id = auth.uid() OR (workspace_id IS NOT NULL AND public.is_workspace_owner(workspace_id, auth.uid())));
CREATE POLICY "Creators or workspace owners can delete tasks" ON public.tasks FOR DELETE TO authenticated USING (creator_id = auth.uid() OR (workspace_id IS NOT NULL AND public.is_workspace_owner(workspace_id, auth.uid())));
...
CREATE POLICY "Task viewers can read task members" ON public.task_members FOR SELECT TO authenticated USING (user_id = auth.uid() OR public.can_view_task(task_id, auth.uid()));
CREATE POLICY "Task creators can manage task members" ON public.task_members FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM public.tasks WHERE id = task_id AND creator_id = auth.uid()));
CREATE POLICY "Task creators can remove task members" ON public.task_members FOR DELETE TO authenticated USING (user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.tasks WHERE id = task_id AND creator_id = auth.uid()));
...
CREATE POLICY "Task viewers can read task progress" ON public.task_progress FOR SELECT TO authenticated USING (user_id = auth.uid() OR public.can_view_task(task_id, auth.uid()));
CREATE POLICY "Users can insert own task progress" ON public.task_progress FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid() AND public.can_view_task(task_id, auth.uid()));
CREATE POLICY "Users can update own task progress" ON public.task_progress FOR UPDATE TO authenticated USING (user_id = auth.uid() AND public.can_view_task(task_id, auth.uid())) WITH CHECK (user_id = auth.uid() AND public.can_view_task(task_id, auth.uid()));
...
CREATE POLICY "Users can view permitted focus sessions" ON public.focus_sessions FOR SELECT TO authenticated USING (user_id = auth.uid() OR (task_id IS NOT NULL AND EXISTS (SELECT 1 FROM public.tasks t WHERE t.id = task_id AND (t.creator_id = auth.uid() OR EXISTS (SELECT 1 FROM public.task_members tm WHERE tm.task_id = t.id AND tm.user_id = auth.uid()) OR (t.scope = 'workspace' AND EXISTS (SELECT 1 FROM public.workspace_members wm WHERE wm.workspace_id = t.workspace_id AND wm.user_id = auth.uid()))))) OR (workspace_id IS NOT NULL AND EXISTS (SELECT 1 FROM public.workspace_members wm WHERE wm.workspace_id = public.focus_sessions.workspace_id AND wm.user_id = auth.uid())));
CREATE POLICY "Users can insert own focus sessions" ON public.focus_sessions FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update own focus sessions" ON public.focus_sessions FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
